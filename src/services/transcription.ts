...
export const processAudioWithSummary = async (
  ...
  try {
    ...
    const { transcription, summary, fileUrl } = await processAudioInSupabase(audioFile); // Include fileUrl in return

    // Capture userId in a local variable right before insert
    const currentUserId = userId; // ADDED: Capture userId locally

    // Save the note with transcription and summary, using insert().select()
    const { data: noteData, error: noteError } = await supabase
      .from('notes')
      .insert({
        user_id: currentUserId, // MODIFIED: Use currentUserId here
        title: metadata.title,
        transcription,
        raw_summary: summary, // Store raw summary
        audio_url: fileUrl,     // Store audio URL
      })
      .select() // MODIFIED: Use .select() instead of .single()
      .single(); // Keep .single() to ensure we get a single record back

    console.log('processAudioWithSummary: noteData after insert:', noteData);
    console.log('processAudioWithSummary: noteError after insert:', noteError);
    ...
    // noteData should now contain the inserted row, including id
    return {
      transcription,
      summary,
      noteId: noteData?.id, // Access id from noteData (after select, it should be an array)
    };
  } catch (error) {
