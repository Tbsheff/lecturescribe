import { TranscriptionProvider, TranscriptionResult, TranscriptionOptions, AudioFile, TranscriptionConfig, TranscriptionProviderType } from './transcriptionTypes.ts';
import { DeepgramProvider } from './deepgramProvider.ts';
...
export function createTranscriptionManager(): TranscriptionManager {
  const config: TranscriptionConfig = {
    provider: (Deno.env.get('TRANSCRIPTION_PROVIDER') as TranscriptionProviderType) || 'gemini',
    deepgram: {
    ...
    fallbackProvider: (Deno.env.get('TRANSCRIPTION_FALLBACK_PROVIDER') as TranscriptionProviderType) || undefined
  };
